...
\chapter{Архітектура та методологія побудови системи керованої колоризації}
    ...
    \section{Загальна концепція та конвеєр обробки даних системи}
        ...
        \subsection{Стратегія симуляції підказок користувача}
            Оскільки під час реального використання кількість та розташування точок є випадковими,
            ...
            \begin{enumerate}
                ...
                \item Випадковим чином визначається кількість підказок $K$ для поточного знімка з дискретного геометричного розподілу $K \sim \mathrm{Geom}(p=0.2)$, в якому $M[K] = \frac{1 - p}{p}$.
                \item Генерується множина випадкових просторових координат $P = \{(h_i, w_i)\}_{i=1}^{K}$. Для забезпечення стійкості моделі до різних сценаріїв введення, координати обираються випадково по всій сітці зображення.
                \item Формується бінарна маска $M$: у координатах з множини $P$ виставляється $1$, у решті --- $0$.
                \item Формуються канали підказок $a$ та $b$: у точках маски копіюються істинні значення з матриці $Y$, а всі інші пікселі занулюються.
            \end{enumerate}

        \subsection{Конвеєр обробки на етапі інференсу}
            ...
            \begin{enumerate}
                \item Користувач фіксує початковий набір колірних точок (або запускає первинний прохід без них при $K=0$) на монохромній матриці зображення.
                \item Графічний інтерфейс формує відповідний триканальний тензор підказок $I_{\mathrm{hints}}$ та бінарну маску $M$, після чого конкатенує їх із каналом яскравості $I_L$.
                \item Об'єднаний чотириканальний тензор $X_{\mathrm{in}}$ передається в модель для виконання одного прямого проходу (forward pass), за результатами якого система миттєво повертає фінальне забарвлене зображення.
                \item Якщо отриманий результат потребує візуального коригування, користувач дискретно змінює конфігурацію орієнтирів (додає нові точки, видаляє або змінює положення існуючих) та ініціює новий цикл генерації.
            \end{enumerate}
            
            Завдяки лінійній обчислювальній складності блоків $\mathrm{SSM}$ у шийці архітектури, кожен такий поодинокий прохід виконується з мінімальною апаратною затримкою (інференс триває лічені мілісекунди), що забезпечує швидку зміну та високу швидкість порівняння різних варіацій розфарбовування.
